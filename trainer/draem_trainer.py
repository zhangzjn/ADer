...
class DRAEMTrainer(BaseTrainer):
    def __init__(self, cfg):
        super(DRAEMTrainer, self).__init__(cfg)
    ...
    def forward(self):
        self.gray_rec, self.out_mask_sm = self.net(self.imgs)
    ...
    def optimize_parameters(self):
        ...
        with self.amp_autocast():
            self.forward()
            loss_mse = self.loss_terms['mse'](self.gray_rec, self.ori_imgs)
            loss_ssim = self.loss_terms['ssim'](self.gray_rec, self.ori_imgs)
            loss_focal = self.loss_terms['focal'](self.out_mask_sm, self.imgs_mask)
            loss = loss_mse + loss_ssim + loss_focal

...
class PyramidFlowTrainer(BaseTrainer):
	def __init__(self, cfg):
		super(PyramidFlowTrainer, self).__init__(cfg)
		cfg.trainer.data.batch_size_per_gpu = 1
		self.val_loader, self.test_loader = get_loader(cfg)

	def set_input(self, inputs):
		self.imgs = inputs['img'].cuda()
		self.imgs_mask = inputs['img_mask'].cuda()
		trans = torchvision.transforms.Resize(self.cfg.test_size)
	...
	def forward(self):
		self.diff_pixels = self.net(self.imgs)

...
class cfg(cfg_common, cfg_dataset_default, cfg_model_vitad):
	...
	def __init__(self):
		...
		checkpoint_path = 'model/pretrain/dino_deitsmall16_pretrain.pth'
		self.model_t = Namespace()
		self.model_t.name = 'vit_small_patch16_224_dino'
		self.model_t.kwargs = dict(pretrained=True, checkpoint_path='', pretrained_cfg=dict(), pretrained_strict=False, strict=True, img_size=self.size, teachers=[3, 6, 9], neck=[12])

...
class cfg(cfg_common, cfg_dataset_default, cfg_model_cfa):
	...
	def __init__(self):
		...
		self.data.train_transforms = [
			dict(type='Resize', size=(256, 256), interpolation=Image.ANTIALIAS),
		...
		self.data.target_transforms = [
			dict(type='Resize', size=(256, 256), interpolation=Image.NEAREST),
			dict(type='CenterCrop', size=(self.size, self.size)),
			dict(type='ToTensor'),
		]

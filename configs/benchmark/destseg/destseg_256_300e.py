...
class cfg(cfg_common, cfg_dataset_default, cfg_model_destseg):
	...
	def __init__(self):
		...
		self.model_t.kwargs = dict(pretrained=True, checkpoint_path='', strict=False, features_only=True,
								   out_indices=[1, 2, 3])
		self.model_s = Namespace()
		self.model_s.name = 'timm_resnet18'
		self.model_s.kwargs = dict(pretrained=False, checkpoint_path=None, strict=False, features_only=True,
								   out_indices=[1, 2, 3, 4])

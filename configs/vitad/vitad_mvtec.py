...
class cfg(cfg_common, cfg_dataset_default, cfg_model_vitad):
	...
	def __init__(self):
		...
		self.model_t.kwargs = dict(pretrained=True, checkpoint_path='', pretrained_cfg=dict(file='model/pretrain/dino_deitsmall16_pretrain.pth'), pretrained_strict=False,strict=True, img_size=self.size, teachers=[3, 6, 9], neck=[12])
